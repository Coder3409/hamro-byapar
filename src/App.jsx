...
function Header({ lang, setLang, online, t, page, setMenuOpen }) {
  return <header className="topbar"><button className="menu-button" onClick={() => setMenuOpen(true)} aria-label={t('more')}><Menu /></button><div className="mobile-logo"><Logo compact /></div><div className="page-heading"><span>{t(page)}</span></div><div className="topbar-actions"><div className={`connection ${online ? '' : 'offline'}`}>{online ? <span className="dot" /> : <CloudOff size={14} />}<span>{t(online ? 'online' : 'offline')}</span></div><div className="language-switch" aria-label={t('language')}><button className={lang === 'en' ? 'active' : ''} onClick={() => setLang('en')}>EN</button><button className={lang === 'ne' ? 'active' : ''} onClick={() => setLang('ne')}>ने</button></div><button className="notification" aria-label={t('alerts')}><Bell size={19} /><i /></button><div className="avatar">AS</div></div></header>;
}
