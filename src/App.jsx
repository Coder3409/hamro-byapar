...
import {
  AlertTriangle, ArrowRight, BarChart3, Bell, Bot, Box, Check, ChevronRight, CircleDollarSign,
  CloudOff, IndianRupee, LayoutDashboard, Lightbulb, Menu, Package, Plus, Search, Settings,
  ShoppingBag, ShoppingCart, Sparkles, Store, TrendingDown, TrendingUp, UserRound, Wallet, X,
} from 'lucide-react';
...
function Modal({ title, onClose, children }) {
  return <div className="modal-backdrop" onMouseDown={(e) => e.target === e.currentTarget && onClose()}><section className="modal-card" role="dialog" aria-modal="true" aria-label={title}><header><h2>{title}</h2><button className="icon-button" onClick={onClose} aria-label="Close"><X size={20} /></button></header>{children}</section></div>;
}
...
function Header({ lang, setLang, online, t, page, setMenuOpen }) {
  return <header className="topbar"><button className="menu-button" onClick={() => setMenuOpen(true)}><Menu /></button><div className="mobile-logo"><Logo compact /></div><div className="page-heading"><span>{t(page)}</span></div><div className="topbar-actions"><div className={`connection ${online ? '' : 'offline'}`}>{online ? <span className="dot" /> : <CloudOff size={14} />}<span>{t(online ? 'online' : 'offline')}</span></div><div className="language-switch" aria-label={t('language')}><button className={lang === 'en' ? 'active' : ''} onClick={() => setLang('en')}>EN</button><button className={lang === 'ne' ? 'active' : ''} onClick={() => setLang('ne')}>ने</button></div><button className="notification"><Bell size={19} /><i /></button><div className="avatar">AS</div></div></header>;
}

function Sidebar({ page, setPage, t, open, setOpen }) {
  return <><aside className={`sidebar ${open ? 'open' : ''}`}><div className="sidebar-brand"><Logo /></div><nav>{nav.map(([id, Icon, key]) => <button key={id} className={page === id ? 'active' : ''} onClick={() => { setPage(id); setOpen(false); }}><Icon size={19} /><span>{t(key)}</span>{page === id && <i />}</button>)}</nav><div className="sidebar-help"><Sparkles size={18} /><strong>{t('ai')}</strong><p>{t('localInsight')}</p><button onClick={() => setPage('ai')}>{t('askAi')} <ArrowRight size={14} /></button></div><div className="sidebar-profile"><div className="avatar">AS</div><div><strong>Asha Shrestha</strong><span>Asha Kirana Pasal</span></div><ChevronRight size={17} /></div></aside>{open && <button className="sidebar-scrim" aria-label="Close menu" onClick={() => setOpen(false)} />}</>;
}
...
function App() {
  ...
  const saveSale = ({ selected, quantity, price, discount, total }) => { const qty = Number(quantity); const sale = { id: crypto.randomUUID(), productId: selected.id, productName: selected.name, productNameNe: selected.nameNe, quantity: qty, unitPrice: Number(price), discount: Number(discount), total, cost: selected.purchasePrice * qty, createdAt: new Date().toISOString() }; setSales((old) => [...old, sale]); setProducts((old) => old.map((p) => p.id === selected.id ? { ...p, stock: Math.max(0, p.stock - qty), sold: (p.sold || 0) + qty } : p)); setModal(null); notify(t('saleAdded')); };
  ...
  const loadDemo = () => { setProducts(demoProducts.map((p) => ({ ...p }))); setSales(createDemoSales()); setProfileState(demoProfile); setPage('dashboard'); notify(t('demoLoaded')); };
  const pageProps = { sales, products, profile, lang, t, setPage };
  const content = page === 'dashboard' ? <Dashboard {...pageProps} openSale={() => setModal({ type: 'sale' })}/> : page === 'sales' ? <SalesPage {...pageProps} openSale={() => setModal({ type: 'sale' })} quickSell={(product) => setModal({ type: 'sale', product })}/> : page === 'inventory' ? <InventoryPage {...pageProps} addProduct={() => setModal({ type: 'product' })}/> : page === 'ai' ? <AIPage {...pageProps}/> : page === 'settings' ? <SettingsPage {...pageProps} setProfile={updateProfile} loadDemo={loadDemo}/> : <div className="page fade-in"><section className="page-title"><div><span className="eyebrow">{t('analytics')}</span><h1>{t('salesTrend')}</h1><p>{officialSlogan}</p></div></section><SalesChart sales={sales} lang={lang} t={t}/><div className="two-column"><ProductRanking products={products} t={t} lang={lang}/><ProductRanking products={products} t={t} lang={lang} slow/></div></div>;
  if (!ready) return <div className="loading-screen"><Logo/><div className="loading-bar"><i/></div><span>{officialSlogan}</span></div>;
  return <div className="app-shell"><Sidebar page={page} setPage={setPage} t={t} open={menuOpen} setOpen={setMenuOpen}/><main><Header lang={lang} setLang={setLang} online={online} t={t} page={page} setMenuOpen={setMenuOpen}/>{content}<footer>{officialSlogan}</footer></main><nav className="mobile-nav">{[['dashboard',LayoutDashboard,'home'],['sales',ShoppingCart,'sales'],['inventory',Package,'inventory'],['ai',Bot,'ai'],['settings',Settings,'more']].map(([id,Icon,key]) => <button key={id} className={page === id ? 'active' : ''} onClick={() => setPage(id)}><Icon/><span>{t(key)}</span></button>)}</nav>{modal?.type === 'sale' && <SaleModal products={products} lang={lang} t={t} preset={modal.product} onClose={() => setModal(null)} onSave={saveSale}/>} {modal?.type === 'product' && <ProductModal t={t} onClose={() => setModal(null)} onSave={saveProduct}/>}<Toast message={toast}/></div>;
}
