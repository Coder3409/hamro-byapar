...
import {
  ...
  CloudOff, LayoutDashboard, Lightbulb, Menu, MonitorUp, Package, Plus, Search, Settings,
...
function App() {
  ...
  const content = page === 'dashboard' ? <Dashboard {...pageProps} openSale={() => setModal({ type: 'sale' })}/> : page === 'sales' ? <SalesPage {...pageProps} openSale={() => setModal({ type: 'sale' })} quickSell={(product) => setModal({ type: 'sale', product })}/> : page === 'inventory' ? <InventoryPage {...pageProps} addProduct={() => setModal({ type: 'product' })}/> : page === 'ai' ? <AIPage {...pageProps}/> : page === 'settings' ? <SettingsPage {...pageProps} presentation={presentation} setPresentation={setPresentation} setProfile={updateProfile} loadDemo={loadDemo}/> : <div className="page fade-in"><section className="page-title"><div><span className="eyebrow">{t('analytics')}</span><h1>{t('salesTrend')}</h1><p>{officialSlogan}</p></div></section><SalesChart sales={sales} lang={lang} t={t}/><div className="two-column"><ProductRanking products={products} t={t} lang={lang}/><ProductRanking products={products} t={t} lang={lang} slow/></div></div>;
